		...
		case NEWLINE:
			{
				...
				if ( (this_tok_type == NEWLINE || this_tok_type == SEMICOLON) && next_tok_type == OTHER) 
				{
					...
					//the only thing that can follow operaters are simple commands and the beginnings of subshells
...
struct linked_list* create_token_list(char* buffer)
{
	...
	while (tok_list->head->child->tok_type == NEWLINE) 
		RemoveAtHead(tok_list);
